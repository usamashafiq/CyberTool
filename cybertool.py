...
try:
    def update():
        ...
        if not nouse:
            if "Already up to date" in use.decode():
                ...
                print(use.decode())
                print("\u001b[32m[+] cybertool is UPDATED To Latest Version")
                try:
                    subprocess.run('cybertool',shell=True, check = True)
                except Exception as err:
                    os.system("python3 cybertool.py")
                exit()
            else:
    ...
    def starting():
        parser = argparse.ArgumentParser(
            description="cybertool is a complete resource hub for Cyber Security Community. Our aim is to make this "
        ...
        ip_args.add_argument("--ipinfo", "-ipi", action="store_true", help="Get IP Infomation(Options: --ip,--file,"
                                                                           "--output)")
        ...
        dns_Args.add_argument(
            "--dnsrecord", "-dns", action="store_true", help="To Get DNS Records(options: --domain,--file,--output) "
                                                             "and use --record to specify record name "
        )
        dns_Args.add_argument(
    ...
    def main():
        ...
        if "root" not in there:
            try:
                subprocess.run('sudo cybertool',shell=True, check = True)
            except Exception as err:
                os.system("sudo python3 cybertool.py")
            # os.system("sudo cybertool")
            exit()
        while True:
            os.system("clear")
            banner.main()
